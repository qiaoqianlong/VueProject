<template>
  <div>
    <div style="margin: 10px 0">
      <el-input style="width: 200px" placeholder="请输入班级名称" suffix-icon="el-icon-search" v-model="name"></el-input>
    ...
    <div style="margin: 10px 0">
      ...
      <el-upload :action="'http://' + serverIp + ':9090/clazz/import'" :show-file-list="false" accept="xlsx" :on-success="handleExcelImportSuccess" style="display: inline-block">
...
export default {
  ...
  methods: {
    ...
    save() {
      this.request.post("/clazz", this.form).then(res => {
        if (res.code === '200') {
    ...
    del(id) {
      this.request.delete("/clazz/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/clazz/del/batch", ids).then(res => {
        if (res.code === '200') {
    ...
    exp() {
      window.open(`http://${serverIp}:9090/clazz/export`)
    },
